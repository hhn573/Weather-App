...
function showTemperatureChart(forecastData) {
    // Kiểm tra xem phần tử canvas có tồn tại hay không
    const canvas = document.getElementById('temperatureChart');
    if (!canvas) {
        console.error('Không tìm thấy phần tử canvas với id "temperatureChart".');
        return;
    }
    ...
    if (!forecastData || forecastData.length === 0) {
        console.error('Dữ liệu dự báo không hợp lệ hoặc trống.');
        return;
    }

    // Lấy dữ liệu nhiệt độ từ dữ liệu dự báo
    const labels = forecastData.map(item => {
        const date = new Date(item.dt_txt);
        return `${date.getDate()}/${date.getMonth() + 1}`;
    });
    const temperatures = forecastData.map(item => Math.round(item.main.temp - 273.15)); // Chuyển đổi từ Kelvin sang Celsius

    // Nếu biểu đồ đã tồn tại, cập nhật dữ liệu mới
    if (temperatureChart) {
...
// Sự kiện click để hiển thị biểu đồ
fiveDaysForecastCard.addEventListener('click', function() {
    // Kiểm tra lại xem biến fiveDaysForecast đã được khởi tạo và có dữ liệu
    if (!fiveDaysForecast || fiveDaysForecast.length === 0) {
        console.error('Biến fiveDaysForecast không có dữ liệu.');
        return;
    }
    showTemperatureChart(fiveDaysForecast);
});
